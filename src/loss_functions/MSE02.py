...
class MSE(LossFunctionParent):
    ...
    def compute_loss(self, input_dict: dict, output_dict: dict, return_loss_maps: bool = False):
        ...
        if self.augmented_coarse_mlp_needed and ('points_augmentation_rgb_coarse' in output_dict):
            aug_pred_rgb_coarse = output_dict['points_augmentation_rgb_coarse']
        ...
        if self.augmented_fine_mlp_needed and ('points_augmentation_rgb_fine' in output_dict):
            aug_pred_rgb_fine = output_dict['points_augmentation_rgb_fine']
