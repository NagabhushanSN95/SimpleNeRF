...
class SimpleNeRF(torch.nn.Module):
    def __init__(self, configs: dict, model_configs: dict):
        ...
        self.predict_visibility = (self.coarse_mlp_needed and self.configs['model']['coarse_mlp']['predict_visibility']) or \
                                  (self.fine_mlp_needed and self.configs['model']['fine_mlp']['predict_visibility'])

        self.coarse_model = None
        self.fine_model = None

        self.points_augmentation_needed = 'points_augmentation' in configs['model']
        if self.points_augmentation_needed:
            self.pts_aug_coarse_mlp_needed = 'coarse_mlp' in self.configs['model']['points_augmentation']
            self.pts_aug_fine_mlp_needed = 'fine_mlp' in self.configs['model']['points_augmentation']
            if self.pts_aug_coarse_mlp_needed:
                self.pts_aug_coarse_model = None
            if self.pts_aug_fine_mlp_needed:
                self.pts_aug_fine_model = None

        self.views_augmentation_needed = 'views_augmentation' in configs['model']
        if self.views_augmentation_needed:
            self.views_aug_coarse_mlp_needed = 'coarse_mlp' in self.configs['model']['views_augmentation']
            self.views_aug_fine_mlp_needed = 'fine_mlp' in self.configs['model']['views_augmentation']
            if self.views_aug_coarse_mlp_needed:
                self.views_aug_coarse_model = None
            if self.views_aug_fine_mlp_needed:
                self.views_aug_fine_model = None
        self.build_nerf()
        return
    ...
    def build_nerf(self):
        ...
        if self.points_augmentation_needed:
            if self.pts_aug_coarse_mlp_needed:
                self.pts_aug_coarse_model = MLP(
                    self.configs, self.configs['model']['points_augmentation']['coarse_mlp']
                )
            if self.pts_aug_fine_mlp_needed:
                self.pts_aug_fine_model = MLP(self.configs, self.configs['model']['points_augmentation']['fine_mlp'])

        if self.views_augmentation_needed:
            if self.views_aug_coarse_mlp_needed:
                self.views_aug_coarse_model = MLP(self.configs, self.configs['model']['views_augmentation']['coarse_mlp'])
            if self.views_aug_fine_mlp_needed:
                self.views_aug_fine_model = MLP(self.configs, self.configs['model']['views_augmentation']['fine_mlp'])
        return
    ...
    def render_rays(self, input_dict: dict, retraw, sec_views_vis):
        ...
        if (self.coarse_mlp_needed and self.configs['model']['coarse_mlp']['use_view_dirs']) or \
                (self.fine_mlp_needed and self.configs['model']['fine_mlp']['use_view_dirs']):
        ...
        if self.coarse_mlp_needed:
            ...
            if self.points_augmentation_needed and self.pts_aug_coarse_mlp_needed and self.training:
                network_output_coarse = self.run_network(network_input_coarse, self.pts_aug_coarse_model)
                if not self.ndc:
                    outputs_coarse = self.volume_rendering(network_output_coarse, z_vals=z_vals_coarse, rays_d=rays_d,
                                                           sec_views_vis=sec_views_vis)
                else:
                ...
                for key in outputs_coarse:
                    return_dict[f'points_augmentation_{key}_coarse'] = outputs_coarse[key]
                if retraw:
                    for key in network_output_coarse.keys():
                        return_dict[f'points_augmentation_raw_{key}_coarse'] = network_output_coarse[key]

            if self.views_augmentation_needed and self.views_aug_coarse_mlp_needed and self.training:
                network_output_coarse = self.run_network(network_input_coarse, self.views_aug_coarse_model)
                if not self.ndc:
                    outputs_coarse = self.volume_rendering(network_output_coarse, z_vals=z_vals_coarse, rays_d=rays_d,
                                                           sec_views_vis=sec_views_vis)
                else:
                    outputs_coarse = self.volume_rendering(network_output_coarse, z_vals_ndc=z_vals_coarse,
                                                           ...
                                                           sec_views_vis=sec_views_vis)
                for key in outputs_coarse:
                    return_dict[f'views_augmentation_{key}_coarse'] = outputs_coarse[key]
                if retraw:
                    for key in network_output_coarse.keys():
                        return_dict[f'views_augmentation_raw_{key}_coarse'] = network_output_coarse[key]
        ...
        if self.fine_mlp_needed:
            ...
            if self.points_augmentation_needed and self.pts_aug_fine_mlp_needed and self.training:
                network_output_fine = self.run_network(network_input_fine, self.pts_aug_fine_model)
                if not self.ndc:
                    outputs_fine = self.volume_rendering(network_output_fine, z_vals=z_vals_fine, rays_d=rays_d,
                                                         sec_views_vis=sec_views_vis)
                else:
                ...
                for key in outputs_fine:
                    return_dict[f'points_augmentation_{key}_fine'] = outputs_fine[key]
                if retraw:
                    for key in network_output_fine.keys():
                        return_dict[f'points_augmentation_raw_{key}_fine'] = network_output_fine[key]

            if self.views_augmentation_needed and self.views_aug_fine_mlp_needed and self.training:
                network_output_fine = self.run_network(network_input_fine, self.views_aug_fine_model)
                if not self.ndc:
                    outputs_fine = self.volume_rendering(network_output_fine, z_vals=z_vals_fine, rays_d=rays_d,
                                                         sec_views_vis=sec_views_vis)
                else:
                    outputs_fine = self.volume_rendering(network_output_fine, z_vals_ndc=z_vals_fine,
                                                         rays_d_ndc=rays_d_ndc,
                                                         rays_o=rays_o, rays_d=rays_d, sec_views_vis=sec_views_vis)

                for key in outputs_fine:
                    return_dict[f'views_augmentation_{key}_fine'] = outputs_fine[key]
                if retraw:
                    for key in network_output_fine.keys():
                        return_dict[f'views_augmentation_raw_{key}_fine'] = network_output_fine[key]
